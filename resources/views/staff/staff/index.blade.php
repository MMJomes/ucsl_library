...
@section('delete_route')
    {{ route('staff.staffs.destroy', 'slug') }}
@endsection
                        ...
                        @canany(['member.approve', 'member.mass_approve'])
                            {
                                ...
                                "render": function(data, type, full, meta) {
                                    var sle = full.status;
                                    var approveURL =
                                        "{{ route('staff.staffs.approve', ':slug') }}";
                        ...
                        @canany(['author.edit', 'author.delete', 'author.show'])
                            {
                                ...
                                "render": function(data, type, full, meta) {
                                    var editURL =
                                        "{{ route('staff.staffs.edit', ':slug') }}";
                                    editURL = editURL.replace(':slug', full.slug);

                                    var showURL =
                                        "{{ route('staff.staffs.show', ':slug') }}";
