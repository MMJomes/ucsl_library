...
@section('content')
    <div class="row page-titles">
        <div class="col-md-12">
            <h4 class="text-white">{{ 'Staff PreRequest  Books List' }} </h4>
        </div>
        <div class="col-md-6">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('backend.dashboard.index') }}">{{ __('message.home') }}</a></li>
                <li class="breadcrumb-item Approve active">{{ 'Staff PreRequest Books List' }}</li>
            </ol>
        </div>
    </div>
    <div class="row">
...
@section('delete_route')
    {{ route('staff.requestbyStaffs.destroy', 'slug') }}
@endsection
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            ...
            @endcan
            $.ajax({
                url: "{{ route('staff.requestbyStaffs.index') }}",
                cache: false,
                        ...
                        },
                        {
                        ...
                        },
                            {
                                ...
                                "render": function(data, type, full, meta) {
                                    var sle = full.status;
                                    var approveURL =
                                        "{{ route('staff.requestbyStaffs.approve', ':id') }}";
                            ...
                            },
                          {
                                ...
                                "render": function(data, type, full, meta) {
                                    var showURL =
                                        "{{ route('staff.requestbyStaffs.show', ':id') }}";
