    ...
    <div class="row page-titles">
        ...
        <div class="col-md-6">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Main Business List</li>
            </ol>
        </div>
    </div>
    <div class="row">
                        ...
                        <table id="dataTable" class="display nowrap table table-hover table-striped table-bordered"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>KeyWord</th>
                                    <th>Created Date</th>
                                    @canany(['mainbusiness.edit', 'mainbusiness.delete'])
                                        <th>Actions</th>
                                    @endcanany
                                </tr>
                            </thead>
...
@section('delete_route')
    {{ route('backend.mainbusiness.destroy', 'slug') }}
@endsection
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            @can('mainbusiness.mass_destroy')
                window.route_mass_crud_entries_destroy = "{{ route('backend.mainbusiness.mass.destroy') }}";
            @endcan
            @can('mainbusiness.show')
                window.route_mass_crud_entries_show = "{{ route('backend.mainbusiness.mass.destroy') }}";
            @endcan
            @can('mainbusiness.create')
                var mainbussiness =
                    "{{ route('backend.mainbusiness.create', ':slug') }}";
                mainbussiness = mainbussiness.replace(':slug', slug);
            @endcan

            $.ajax({
                url: "{{ route('backend.mainbusiness.index') }}",
                cache: false,
                ...
                $('#dataTable').DataTable({
                            ...
                            @can('bizcategory.create')
                                {
                                    ...
                                    action: function(e, dt, node, config) {
                                        window.location.href =mainbussiness;
                                    }
                                },
                            @endcan
                        @endif
                    ...
                    columns: [{
                        ...
                        @canany(['mainbusiness.edit', 'mainbusiness.delete', 'mainbusiness.show'])
                            {
                                orderable: false,
                                "render": function(data, type, full, meta) {
                                    var editURL =
                                        "{{ route('backend.mainbusiness.edit', ':slug') }}";
                                    editURL = editURL.replace(':slug', full.slug);

                                    var showURL =
                                        "{{ route('backend.mainbusiness.show', ':slug') }}";
