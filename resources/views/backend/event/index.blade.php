...
@section('delete_route')
    {{ route('backend.event.destroy', 'slug') }}
@endsection
...
@push('scripts')
    <script>
        $(document).ready(function() {
            @can('event.mass_destroy')
                window.route_mass_crud_entries_destroy = "{{ route('backend.event.mass.destroy') }}";
            @endcan
            @can('event.show')
                window.route_mass_crud_entries_show = "{{ route('backend.event.mass.destroy') }}";
            @endcan
            $.ajax({
                url: "{{ route('backend.event.index') }}",
                cache: false,
                ...
                $('#dataTable').DataTable({
                    ...
                    columns: [{
                        ...
                        @canany(['event.edit', 'event.delete','event.show'])
                            {
                                orderable:false,
                                "render": function(data, type, full, meta) {
                                    var editURL =
                                        "{{ route('backend.event.edit', ':slug') }}";
                                    editURL = editURL.replace(':slug', full.slug);

                                    var showURL =
                                        "{{ route('backend.event.show', ':slug') }}";
